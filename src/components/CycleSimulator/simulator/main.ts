...
import { ERROR_ACTION, 根据奇穴修改buff数据, 起手识破BUFF } from './utils'
import {
  技能GCD组,
  技能类实例集合,
  检查运行数据实例类型,
...
class 循环主类 {
  ...
  技能释放后更新运行数据(当前技能: CycleSimulatorSkillDTO, 技能实例: 检查运行数据实例类型) {
    if (技能实例?.技能运行数据) {
      ...
      const 充能满第一次释放时间点 = 技能实例?.技能运行数据?.充能满第一次释放时间点
      const 释放后层数 = 技能实例?.技能运行数据?.当前层数 - 1
      if (是否为充满后第一次释放) {
        if (释放后层数 <= 0) {
          技能实例?.更新技能运行数据({
            计划下次充能时间点: (this.当前时间 || 0) + (当前技能?.技能CD || 0),
            当前层数: 释放后层数,
          })
        } else {
          技能实例?.更新技能运行数据({
            当前层数: 释放后层数,
            充能满第一次释放时间点: this.当前时间,
          })
        }
      } else if (释放后层数 <= 0) {
        技能实例?.更新技能运行数据({
          计划下次充能时间点: (充能满第一次释放时间点 || 0) + (当前技能?.技能CD || 0),
          当前层数: 释放后层数,
        })
      } else {
        技能实例?.更新技能运行数据({
  ...
  触发避实击虚() {
    ...
    if (破绽避实击虚枚举[破绽层数]) {
      this.技能造成伤害(`避实击虚`, 破绽避实击虚枚举[破绽层数])
    }
  }
