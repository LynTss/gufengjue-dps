...
export const getSingleSkillTotalDps = (
  ...
  if (当前技能属性) {
    ...
    if (人物属性?.套装会心会效) {
      // 偷懒覆盖率测试80%左右
      郭氏额外会效果值 = 郭氏额外会效果值 + 41 * 0.8
      额外会心率 = 额外会心率 + 0.04 * 0.8
    }

    if (人物属性?.切糕双会 && 人物属性?.切糕双会 > 0) {
      最终人物属性 = {
        ...最终人物属性,
        会心值: 最终人物属性?.会心值 + 1090 * 人物属性?.切糕双会,
      }
    }

    if (人物属性?.切糕无双 && 人物属性?.切糕无双 > 0) {
      最终人物属性 = {
        ...最终人物属性,
        无双值: 最终人物属性?.无双值 + 1090 * 人物属性?.切糕无双,
      }
    }
    ...
    if (人物属性?.水特效武器) {
      最终人物属性 = {
        ...最终人物属性,
        基础攻击: 最终人物属性?.基础攻击 + 670, // 按覆盖率100%算 一层67 10层670
        面板攻击: 最终人物属性?.面板攻击 + 670, // 按覆盖率100%算 一层67 10层670
      }
    }
