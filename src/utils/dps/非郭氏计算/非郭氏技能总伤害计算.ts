...
// 计算技能循环总输出
export const 非郭氏技能总伤害计算 = (props: 技能总伤害计算入参类型) => {
  const {
    计算循环,
    角色最终属性,
    当前目标,
  ...
  // 获取装备增益等带来的最终增益集合
  let 总增益集合: SKillGainData[] = 获取全部增益(计算属性, 默认增益集合)
  ...
  最终循环.forEach((item) => {
    // 获取循环内某个技能的总dps
    const { totalDps } = 计算循环内某技能总伤害(item, 计算属性, 计算目标, 技能基础数据, 总增益集合)
    dpsList.push({
      name: item.技能名称,
      dps: totalDps,
      number: item.技能数量,
...
export const 计算技能总伤 = (
  ...
  // 郭式力道对增益提供的力道二次加成提升值
  const 郭式力道对增益内身份的提升值 = 增益计算基础?.力道数值加成 * 力道提升百分比
  // 力道数值的提升值
  const 增益内力道提升值 = 增益计算基础?.力道数值加成 + 郭式力道对增益内身份的提升值
